// src/modules/guiders/guider.repository.js
...
// ═══════════════════════════════════════════════════════════════
// ✅ FIX B-13: Consistent `Op.overlap` for JSONB placeIds
...
export const getAllGuiders = async ({
  ...
  // ✅ JSONB overlap — matches if placeId exists anywhere in array
...
export const getGuiderPlaces = async (guiderId) => {
  ...
  const places = await Place.findAll({
    where: { id: { [Op.in]: guider.placeIds } },
  });
  return places;
};
...
export const removeGalleryImage = async (guiderId, imageUrl) => {
  const guider = await Guider.findByPk(guiderId);
  if (!guider) return null;
  const gallery = (Array.isArray(guider.gallery) ? guider.gallery : []).filter(
    (url) => url !== imageUrl
  );
  await guider.update({ gallery });
  return guider;
